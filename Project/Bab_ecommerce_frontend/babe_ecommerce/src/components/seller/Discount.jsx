...
function Discount() {
  ...
  const fetchDiscounts = async (page = 1) => {
    try {
      const response = await get(`${endpoints.getAllCoupons}?sellerId=${userId}&status=${status}&page=${page}`);
    ...
    <div className="p-6">
          <div className="mb-6 flex items-center justify-between satosi_regular">
            ...
            <button
              ...
              onClick={() => navigate('/seller/create_discount')}
            >
              Create New Discount
            </button>
                    ...
                    discounts.map((item) => (
                      <tr key={item.id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="py-2 px-3 flex gap-2">
                          <FaEdit className="text-blue-500 cursor-pointer" onClick={() => navigate(`/seller/edit_discount/${item.id}`)} />
