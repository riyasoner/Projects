...
const Orders = () => {
  ...
  const [showTrackeModel, setshowTrackModel] = useState(false)
  ...
  const fetchOrders = async () => {
    try {
      const response = await get(`${endpoints.getOrdersBySellerId}/${userId}?page=${currentPage}`);
  ...
  const [courierData, setCourierData] = useState({
    courierName: "",
    trackingNumber: orders?.trackingNumber || null,
    trackingUrl: "",
  });



  const postCourierData = async () => {
    try {
      const response = await patch(`${endpoints.tracking}?suborderId=${subOrderId}`, courierData);
      console.log("Courier data posted:", response.data);
      toast.success(response.message)
      setshowTrackModel(false)
    } catch (error) {
      console.error("Error posting courier data:", error);
    }
  };

  const fetchSummary = async () => {
    try {
      const response = await get(`${endpoints.getOrderSummary}?sellerId=${userId}`);
      setSummary(response || {});
    } catch (error) {
  ...
  const exportOrdersToExcel = () => {
    ...
    const worksheetData = orders.map((order, index) => ({
      "S.No": index + 1,
      "Order ID": order.id,
      "Customer Name": order?.order?.user?.fullName || "N/A",
  ...
  const markASDelivered = async (id) => {
    const payload = {
      suborderId: id, sellerId: Number(userId)
    }
    try {
      const res = await patch(endpoints.markSuborderAsDelivered, payload)
  ...
  const handlemarkASDelivered = (id) => {
    confirmAlert({
      title: "Confirm Deletion",
      message: "Are you sure you want to delivered Product",
      buttons: [
        {
          label: "Yes",
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-black text-white px-4 py-2 rounded-lg hover:bg-gray-800" style={{ borderRadius: '10px' }}
        >
        ...
        <table className="min-w-full table-auto">
          <thead className="bg-gray-100 text-left">
            <tr>
              ...
              <th className="p-4">Track Order</th>
              <th className="p-4">Action</th>
              {/* {orders.map((order, index) =>
                order?.order?.paymentMethod === "cod" && (
                  <th key={index}>Delivered</th>
                )
              )} */}
            </tr>
          </thead>
          <tbody>
            {(orders.length > 0) ? orders.map((order) => (
              <tr key={order.id} className="border-t hover:bg-gray-50">
                ...
                <td className="p-4">
                  <p className="font-semibold">{order?.order?.user?.fullName}</p>
                  <p className="text-sm text-gray-500">{order.user?.email}</p>
                </td>
                <td className="p-4">
                  <span className={`px-3 py-1 rounded-full text-sm ${order?.order.paymentStatus === "pending"
                ...
                <td className="p-4 space-y-2 w-40"> {/* Adjust width if needed */}
                  {/* Track Button */}
                  <button
                    className="w-full h-10 flex items-center justify-center gap-2 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700 transition duration-150 shadow-sm"
                    onClick={() => {
                      setshowTrackModel(true);
                      setSubOrderId(order.id);
                    }}
                  ...
                  {/* Delivered Button for COD */}
                  {order?.order?.paymentMethod === "cod" && (
                    <button
                      className="w-full mt-2 h-10 flex items-center justify-center gap-2 bg-green-700 text-white text-sm rounded-md hover:bg-green-800 transition duration-150 shadow-sm"
                ...
                <td className="p-4 flex gap-3">
                  <button className="text-purple"
                    onClick={() => {
                      navigate("/seller/view_order", { state: { id: order.orderId } })
                    }}>
                    <FaEye />
                  </button>
      ...
      {showTrackeModel && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="relative bg-white p-6 rounded-lg w-96 shadow-lg">
            <h3 className="text-xl font-bold mb-4">Order Tracker</h3>
            <button
              onClick={() => setshowTrackModel(false)}
            ...
            <div className="mb-3">
              <label className="block text-sm font-medium">Tracking URL</label>
              <input
                type="text"
