...
const EditCoinRule = () => {
    ...
    const [coinData, setCoinData] = useState(initialValues);
    ...
    const fetchCoinByid = async () => {
        try {
            const api = await get(`${endpoints.getCoinRuleById}/${id}`);
            ...
            if (response) {
                setCoinData({
                    ...
                    coinsPerAction: response?.coinsPerAction || "",
                    perAmount: response?.perAmount || "",
    ...
    const handleSubmit = async (values, { resetForm }) => {
        try {
            ...
            const response = await patch(`${endpoints.updateCoinRule}/${id}`, payload);
    ...
    return (
        <div className="max-w-lg mx-auto p-6 bg-white rounded shadow mt-10">
            ...
            <h2 className="text-2xl font-bold mb-6">Edit Coin Rule</h2>

            <Formik
                initialValues={coinData}
