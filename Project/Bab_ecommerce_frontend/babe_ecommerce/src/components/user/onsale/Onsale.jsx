...
import { FaFlagCheckered, FaHeart, FaRegHeart, FaTimesCircle } from "react-icons/fa";
...
const Onsale = () => {
  ...
  const fetchProducts = async (pageNum) => {
    try {
      const res = await get(`${endpoints.getOnSaleProducts}?page=${pageNum}&limit=${LIMIT}`);
  ...
  return (
    <>
      ...
      <section id="newarrivals" className="py-12">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-6 mt-4">
            {products.length === 0 ? (<div className="col-span-full flex justify-center items-center py-10">
              <EmptyProducts />
          ...
          {!hasMore && (
            <p className="text-center mt-6 text-gray-600 text-lg font-semibold flex items-center justify-center gap-2">
