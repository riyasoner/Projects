...
const EditDiscount = () => {
    ...
    const { id } = useParams();
    const [discount, setDiscouunt] = useState(null)
    useEffect(() => {
        fetchDiscount()
    }, [])
    const fetchDiscount = async () => {
        try {
            const response = await get(`${endpoints.getCouponById}/${id}`)
            setDiscouunt(response.data || [])

        } catch (error) {
            console.log("Error in fetching Discount By id", error)
        }
    }
    ...
    const initialValues = {
        ...
        appliedOnType: discount?.appliedOnType,
        appliedOnId: discount?.appliedOnId,
    ...
    const handleSubmit = async (values, { resetForm }) => {
        try {
            const payload = {
                ...
                createdByAdmin: userType === "admin"
            };
            const response = await patch(`${endpoints.updateCoupon}/${id}`, payload);
            toast.success(response.message || "Discount Updated");
            resetForm();
            navigate(-1);
        } catch (error) {
