...
const EditFaq = () => {
  ...
  const fetchFaqById=async()=>{
    try {
        const response=await get(`${endpoints.getFAQById}/${id}`);
        setFaq(response.data || {})

        
    } catch (error) {
         console.log("Error For Fetching Faq By Id",error)
    }
  }
  ...
  const handleSubmit = async (values, { resetForm }) => {
    try {
      ...
      const response = await patch(`${endpoints.updateFAQ}/${id}`, payload);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded shadow mt-10">
      ...
      <h2 className="text-2xl font-bold mb-6">Update FAQ</h2>
