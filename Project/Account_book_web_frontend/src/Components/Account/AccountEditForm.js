...
const AccountForm = ({ account = null, onSuccess }) => {
  ...
  useEffect(() => {
    if (account) {
      setFormData({
        ...
        account_type: account.account_type || "PERSONNEL",
        unit: account.unit || "inr",
        bookId: account.bookId || localStorage.getItem("bookId"),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = account
        ? await patch(
            `${endPoints.updateAccountById}/${account.id}`,
            dataToSend
          )
        : await post(endPoints.addAccount, dataToSend);
      ...
      if (response?.data || response?.status) {
        ...
        if (!account) {
          // Only reset form on new account
          setFormData({
            name: "",
            description: "",
            account_type: "PERSONNEL",
        ...
        if (onSuccess) onSuccess(); // trigger fetch or modal close
      } else {
